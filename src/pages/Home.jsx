...
const Home = () => {
  ...
  if (userError)
    return (
      <div className="p-6">
        <p className="text-red-400 mb-3">{userError}</p>
        <div className="flex gap-3">
          <button onClick={() => navigate('/')} className="px-3 py-1 underline">
            Return to Login
          </button>
          <button onClick={() => window.location.reload()} className="px-3 py-1 bg-aqua rounded">
            Retry
          </button>
        </div>
      </div>
  ...
  if (!userObj)
    return (
      <div className="p-6">
        <p>
          No user data available. <button onClick={logout} className="underline">Sign out</button>
        </p>
      </div>
    );
  ...
  return (
    <div className="p-6 mx-auto max-w-7xl">
      <header className="flex items-start justify-between gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-400">
            Welcome back, <span className="text-aqua">{userObj.name}</span>
          </h1>
          <p className="text-sm text-gray-400 mt-1">Here is your personalized feed</p>
        </div>
      </header>
        ...
        <div className="flex gap-3">
          {categories.map((c) => (
            <button
              ...
              className={`px-5 py-2 rounded-full border transition-all duration-300 whitespace-nowrap ${selectedCategory === c.key
                ? 'bg-aqua text-black border-aqua shadow-[0_0_15px_rgba(0,255,255,0.4)] font-bold'
                : 'bg-transparent text-gray-300 border-white/10 hover:border-aqua/50 hover:text-white'
                }`}
            >
              {c.label}
            </button>
      ...
      <section className="mb-12">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-semibold flex items-center gap-2">
            <span className="w-1 h-6 bg-aqua rounded-full"></span>
            Top Stories
          </h2>

          <div className="flex items-center gap-2 bg-gray-900 rounded-lg p-1 border border-white/10">
            <button
              onClick={() => setViewMode('grid')}
              className={`p-2 rounded-md transition-all ${viewMode === 'grid' ? 'bg-white/10 text-aqua' : 'text-gray-400 hover:text-white'}`}
              title="Grid View"
            >
            ...
            <button
              ...
              className={`p-2 rounded-md transition-all ${viewMode === 'list' ? 'bg-white/10 text-aqua' : 'text-gray-400 hover:text-white'}`}
              title="List View"
            >
        ...
        {newsError && <div className="text-red-400 mb-3 p-4 bg-red-900/20 rounded border border-red-900/50">{newsError}</div>}

        {newsLoading ? (
          <div className={`grid gap-6 ${viewMode === 'grid' ? 'sm:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'}`}>
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className={`bg-gray-800/50 animate-pulse rounded-xl ${viewMode === 'grid' ? 'h-64' : 'h-32'}`} />
            ))}
          </div>
        ) : (
          <div className={`grid gap-6 ${viewMode === 'grid' ? 'sm:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'}`}>
            {articles.map((a, idx) => (
              <article
                key={a.id ?? idx}
                className={`bg-gray-900/80 backdrop-blur-sm rounded-xl overflow-hidden border border-white/5 hover:border-aqua/30 transition-all duration-300 hover:shadow-[0_10px_30px_rgba(0,0,0,0.3)] group ${viewMode === 'list' ? 'flex' : ''}`}
              >
                {a.urlToImage ? (
                  <div className={`overflow-hidden ${viewMode === 'list' ? 'w-48 h-full shrink-0' : 'h-48 w-full'}`}>
                  ...
                  <div className={`bg-gray-800 flex items-center justify-center text-gray-500 ${viewMode === 'list' ? 'w-48 h-full shrink-0' : 'h-48 w-full'}`}>
                    <span>No Image</span>
                  </div>
                )}

                <div className="p-5 flex flex-col flex-1">
                  <h3 className="font-bold text-lg mb-2 text-white leading-tight group-hover:text-aqua transition-colors line-clamp-2">{a.title}</h3>
                  <p className="text-sm text-gray-400 line-clamp-2 mb-4 flex-1">{a.description || ''}</p>

                  <div className="flex items-center justify-between mt-auto pt-4 border-t border-white/5">
                    <div className="flex items-center gap-3">
                      <span className="text-xs text-gray-500 font-mono bg-white/5 px-2 py-1 rounded">{a.source || 'Unknown'}</span>
                      <button onClick={() => handleBookmark(a)} className={`transition-colors ${isBookmarked(a) ? 'text-aqua drop-shadow-[0_0_5px_rgba(0,255,255,0.8)]' : 'text-gray-400 hover:text-aqua'}`} title="Bookmark">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        {/* History Widget */}
        <section className="bg-gray-900/50 p-6 rounded-2xl border border-white/5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-white">Quick History</h2>
            <span className="text-xs text-gray-400 bg-white/5 px-2 py-1 rounded-full">{(userObj.history || []).length} items</span>
          </div>
          ...
          <div className="space-y-2 mb-4 max-h-40 overflow-y-auto pr-2 custom-scrollbar">
            {userObj.history && userObj.history.length > 0 ? (
              userObj.history.slice(-5).reverse().map((h, idx) => (
                <div key={idx} className="text-sm text-gray-300 truncate py-1 border-b border-white/5 last:border-0 hover:text-aqua cursor-default">
                  {h}
                </div>
              ))
            ) : (
          ...
          <form onSubmit={handleAddHistory} className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-2 bg-black/20 border border-white/10 rounded-lg outline-none focus:border-aqua/50 text-sm"
            />
            <button type="submit" disabled={updating} className="px-4 py-2 bg-aqua/10 text-aqua hover:bg-aqua hover:text-black rounded-lg transition-all text-sm font-bold disabled:opacity-50">
              Add
            </button>
          </form>
        </section>
        ...
        {/* Bookmarks Widget */}
        <section className="bg-gray-900/50 p-6 rounded-2xl border border-white/5">
          <h2 className="text-lg font-semibold text-white mb-4">Recent Bookmarks</h2>
